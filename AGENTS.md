...
- `dsh-community-market/` owns the community-market shell: catalog discovery, user-directed install and uninstall, and the Market Host and Client faces. Its runtime is implemented, so it declares loadable DSH and package entry points and is consumed by `dsh-plugin-desktop/` as a production dependency. Its source must not import Desktop implementation modules; `yarn check:architecture` enforces that direction.
