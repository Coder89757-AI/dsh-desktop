...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Lexford-Beta-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Lexford Beta.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Lexford Beta.exe: ${portablePath}`)
  }
