...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    ...
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}. After restarting, {product} will use the rolled-back configuration.`,
  ...
  zh: {
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的检查点中的 Profile、共享设置文件 settings.yaml 与 DSH 数据目录补丁；重启后，{product} 将使用回滚后的配置。`,
