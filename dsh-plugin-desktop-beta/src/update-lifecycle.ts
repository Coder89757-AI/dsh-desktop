...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'Lexford 有可用更新', body: `版本 ${version} 已可下载。打开 Lexford 即可继续。` }
    : { title: 'Lexford Update Available', body: `Version ${version} is ready to download. Open Lexford to continue.` }
}
