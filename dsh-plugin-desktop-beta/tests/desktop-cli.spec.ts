...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode and dispatches the imported CLI exactly once', async () => {
    ...
    const argv = ['/Applications/Lexford', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const runCli = vi.fn(async () => {})
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/Lexford',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/Lexford.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/Lexford.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\Lexford\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\Lexford\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/Lexford.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/Lexford.app/Contents/Resources/app.asar.unpacked/package.json')
