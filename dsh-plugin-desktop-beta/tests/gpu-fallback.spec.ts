...
describe('Desktop GPU fallback', () => {
  ...
  it('keeps hardware acceleration when the escape hatch is passed', () => {
    ...
    const logger = failureLogger()
    const disableHardwareAcceleration = vi.fn()
    recordDesktopGpuFailure(statePath, { reason: 'crashed', version: '2.0.10-beta.1' })

    expect(desktopHardwareAccelerationOverridden(['Lexford Beta.exe', DESKTOP_ENABLE_GPU_ARGUMENT])).toBe(true)
    expect(desktopHardwareAccelerationOverridden(['Lexford Beta.exe', `${DESKTOP_ENABLE_GPU_ARGUMENT}=true`])).toBe(false)

    applyDesktopGpuFallback({
      argv: ['Lexford Beta.exe', DESKTOP_ENABLE_GPU_ARGUMENT],
      statePath,
      logger,
      disableHardwareAcceleration,
    })
