...
describe('native macOS application menu', () => {
  ...
  it('localizes the complete Simplified Chinese menu while retaining native roles', () => {
    const template = macApplicationMenuTemplate('Lexford', 'zh-CN')

    expect(template.map(item => item.label)).toEqual([
      'Lexford', '文件', '编辑', '显示', '窗口',
    ])
    expect(submenu(template[0]!).map(item => item.label).filter(Boolean)).toEqual([
      '关于 Lexford', '服务', '隐藏 Lexford', '隐藏其他', '全部显示', '退出 Lexford',
    ])
  ...
  it('keeps the English fallback complete', () => {
    const template = macApplicationMenuTemplate('Lexford', 'en')

    expect(template.map(item => item.label)).toEqual([
      'Lexford', 'File', 'Edit', 'View', 'Window',
    ])
    expect(submenu(template[0]!)).toEqual(expect.arrayContaining([
      expect.objectContaining({ label: 'About Lexford', role: 'about' }),
      expect.objectContaining({ label: 'Quit Lexford', role: 'quit' }),
    ]))
  })
