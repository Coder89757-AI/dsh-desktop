...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Lexford-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Lexford.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Lexford.exe: ${portablePath}`)
  }
