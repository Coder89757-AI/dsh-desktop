...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      ...
      join('/build', 'mac-universal', 'Lexford.app', 'Contents', 'MacOS', 'Lexford'),
      join('/build', 'win-unpacked', 'Lexford.exe'),
      join('/build', 'win-arm64-unpacked', 'Lexford.exe'),
    ].sort()
    ...
    expect(resolveFinalPackagedRuntimeContexts(
    ...
    )).toEqual([
      {
        ...
        packager: {
          ...
          appInfo: { productFilename: 'Lexford' },
        },
      },
      {
        ...
        packager: { appInfo: { productFilename: 'Lexford' } },
      },
      {
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', 'Lexford.exe')
    const stale = join('/build', 'win-arm64-unpacked', 'Lexford.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', 'Lexford.exe')

    expect(() => resolveFinalPackagedRuntimeContexts(
      result([{ key: 'win', archs: [Arch.x64, Arch.arm64] }]),
      filename => filename === x64Executable,
    )).toThrow(`win/arm64 at ${join('/build', 'win-arm64-unpacked', 'Lexford.exe')}`)
  })
  ...
  it('resolves a real target-name map through the target archs retained by NSIS', () => {
    ...
    const built = {
      outDir: '/build',
      configuration: { productName: 'Lexford' },
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', 'Lexford.app', 'Contents', 'MacOS', 'Lexford'),
      join('/build', 'mac', 'Lexford.app', 'Contents', 'MacOS', 'Lexford'),
    ]
    const events: string[] = []
  ...
  it.each([
    ...
    await expect(verifyElectronExecutableFuses('/build/Lexford.exe', read))
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/Lexford.exe', read))
      .rejects.toThrow('/build/Lexford.exe')
  })
})
